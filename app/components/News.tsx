...
export default function News() {
  ...
  return (
    <section className="px-4 py-12 md:px-8 md:py-16 max-w-7xl mx-auto w-full flex flex-col gap-12">
      {/* News Slider Section */}
      <div>
        {/* Section Header */}
        <div className="flex items-center justify-between border-b border-zinc-200 pb-4">
          <div className="flex flex-col gap-1">
            <h2 className="text-xl md:text-3xl font-black font-custom text-[#111] uppercase tracking-wider">
              Dernières actualités de MechaRumble
            </h2>
            <p className="text-zinc-500 text-xs font-bold tracking-wider font-sans uppercase">
              Restez au cœur de l'action !
            </p>
          </div>
          <div className="flex items-center space-x-4">
            <div className="hidden sm:block h-[1px] w-12 bg-zinc-300" />
            <a
              href="#"
              className="flex items-center text-[10px] md:text-xs font-black tracking-widest text-zinc-600 hover:text-[#0e77d9] transition-colors whitespace-nowrap"
            >
        ...
        {/* Scrollable Slider of News Cards inside a hover container group */}
        <div className="relative group mt-8">
          {/* Floating Navigation Controls (Visible on Hover) */}
          <Button
            variant="outline"
            size="icon"
            onClick={() => scroll("left")}
            ...
            >
              ...
              <div className="p-5 flex flex-col justify-between flex-grow">
                <div className="flex flex-col space-y-3">
                  {/* Tag/Year */}
                  <span className="inline-block bg-[#0e77d9] text-white text-[8px] md:text-[9px] font-black tracking-widest px-2.5 py-0.5 rounded w-max select-none">
                    {item.year}
                  </span>
                  {/* Title */}
                  <h3 className="text-[11px] md:text-[12px] font-black text-zinc-900 tracking-wide leading-snug line-clamp-2 uppercase font-sans">
                    {item.title}
                  </h3>
                  {/* Description */}
                  <p className="text-[10px] text-zinc-500 font-bold leading-relaxed line-clamp-3 font-sans">
                    {item.description}
                  </p>
                </div>
              </div>
      ...
      <div className="w-full bg-white border border-zinc-200 rounded-[32px] p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-sm hover:shadow-md transition-shadow">
        <div className="flex flex-col gap-2 max-w-3xl">
          <h3 className="text-base md:text-xl font-black text-zinc-900 uppercase tracking-wider font-sans">
            Abonnez-vous à la newsletter MechaRumble
          </h3>
        ...
        <Button
          variant="gaming"
          className="flex-shrink-0 text-[10px] md:text-[11px] font-black tracking-widest h-12 px-8 rounded-xl uppercase font-sans whitespace-nowrap"
        >
